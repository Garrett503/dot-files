...
PROXYS='/home/garrett/Scripts/dev/proxy.txt'
...
do
  case $OPTION in
    h)
      echo "Usage: $0 [-h - help] [-f <file> - file with proxy, default proxies.txt] [-g <file> - out file for good proxies] [-b <file> - out file for bad proxies] [-u <url> - url for check proxy, default $CHECK_URL] [-m <sec> - max connect time in second, default 10 sec] [-c <count> - max ping count for AVG time, default 4 count]"
...
echo -ne $hi", "$(echo $GEOIP | awk -F, '{print $2}' | awk -F: '{print $2}' | cut -d '"' -f 2) ""
      echo -e $WHT$(ping -c $PING_COUNT $IP | tail -1| awk '{print $4}' | cut -d '/' -f 2)$DEF"ms"
      GOOD=$(($GOOD+1))
      GOOD_ARR+=($PROXY)
    else  
      echo -e $RED"dead"$DEF
